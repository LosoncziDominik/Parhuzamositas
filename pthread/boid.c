...
void update_boids_parallel() {
    ...
    int boids_per_thread = NUM_BOIDS / THREAD_COUNT;

    for (int i = 0; i < THREAD_COUNT; i++) {
        ThreadRange* range = malloc(sizeof(ThreadRange));
        range->start = i * boids_per_thread;
        range->end = (i == THREAD_COUNT - 1) ? NUM_BOIDS : (i + 1) * boids_per_thread;

        int res = pthread_create(&threads[i], NULL, update_boid_range, range);
        if (res != 0) {
...
void* render_boids_range(void* arg){
    ThreadRange* range = (ThreadRange*)arg;
...
void render_boids_paralell(){
    ...
    for (int i = 0; i < THREAD_COUNT; i++) {
        ...
        int res = pthread_create(&threads[i], NULL, render_boids_range, range);
        if (res != 0) {
            fprintf(stderr, "Failed to create thread %d\n", i);
        }
    }
